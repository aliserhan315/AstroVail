...
async function main() {
  ...
  if (!contract) throw new Error("Usage: npx hardhat run scripts/mintStar.ts --network baseSepolia 0xYourContract");
  ...
  const tx = await star.write.mintStar([user.account.address, tokenId, "ipfs://star.json"], { client: { wallet: admin } });
