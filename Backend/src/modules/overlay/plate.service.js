...
export async function solveWithAstrometry(buffer) {
  if (!process.env.ASTROMETRY_KEY) throw new Error("Astrometry API key missing (ASTROMETRY_KEY)");
  ...
  for (let attempt = 1; attempt <= RETRY.tries; attempt++) {
    try {
      ...
      for (let i = 0; i < JOBID_POLLS; i++) {
        ...
        if (Array.isArray(data.jobs)) { const j = data.jobs.find(Boolean); if (j) { jobId = j; break; } }
        await sleep(POLL_MS);
      }
