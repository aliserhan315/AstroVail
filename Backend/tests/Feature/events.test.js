...
describe("Events", () => {
  test("lists events", async () => {
    ...
    const res = await apiGet("/events").expect(200);
  ...
  test("gets by id & remind auth", async () => {
    ...
    await apiGet(`/events/${doc._id}`).expect(200);

    await apiPost(`/events/${doc._id}/remind`).send({ at: new Date().toISOString() }).expect(401);

    const { accessToken } = await registerUser();
    const ok = await apiPost(`/events/${doc._id}/remind`)
