import { apiGet, apiPatch, registerUser, auth } from "../helpers.js";

describe("Me", () => {
  test("GET /me returns current user", async () => {
    const { accessToken, user } = await registerUser();
    const res = await apiGet("/me").set(auth(accessToken)).expect(200);
    expect(res.body.data._id).toBe(user._id);
  });

  test("PATCH /me/profile updates profile", async () => {
    const { accessToken } = await registerUser();
    const res = await apiPatch("/me/profile")
  ...
  test("PATCH /me/device saves tz/location", async () => {
    const { accessToken } = await registerUser();
    const res = await apiPatch("/me/device")
      .set(auth(accessToken))
