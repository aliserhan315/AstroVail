...
describe("Stars", () => {
  test("GET /stars returns random unowned (no q)", async () => {
    ...
    const res = await api().get("/stars").expect(200);
  ...
  test("search by name", async () => {
    ...
    const res = await api().get("/stars?q=vega").expect(200);
  ...
  test("get by id & by catalog", async () => {
    ...
    await api().get(`/stars/${s._id}`).expect(200);
    await api().get(`/stars/by-catalog/HIP 100`).expect(200);
  });

  test("owner-only rename", async () => {
    const { accessToken } = await registerUser();
    const created = await api().post("/stars")
  ...
  test("GET /me/stars shows only my stars", async () => {
    const { accessToken } = await registerUser();
    const mine = await api().post("/stars")
      .set(auth(accessToken))
      .send({ catalogId: `HIP_${Date.now()}`, baseName: "Altair" })
      .expect(201);

    const res = await api().get("/me/stars").set(auth(accessToken)).expect(200);
    const items = res.body.data.items;
